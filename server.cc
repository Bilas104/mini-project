...
int main (void)
{
	...
	coap_address_t dst;
	coap_resource_t * resource = nullptr, * r = nullptr;
	coap_endpoint_t * endpoint = nullptr;
	int result = EXIT_FAILURE;
	coap_str_const_t * ruri = coap_make_str_const("request_1");
	coap_str_const_t * ruri_2 = coap_make_str_const("request_2");
	coap_startup();
	...
	r = coap_resource_init(ruri_2, 0);

	coap_register_handler(r, COAP_REQUEST_GET, [](auto, auto, const coap_pdu_t * request, auto, coap_pdu_t * response)
			{
			rand_txt(43);
